...
# docker-install.sh - install Docker Engine and docker-compose if missing
# Usage: docker-install.sh [service-file-path]

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
SERVICE_SRC="${1:-$SCRIPT_DIR/rover.service}"
...
if ! command -v docker >/dev/null 2>&1; then
    ...
    SERVICE_USER="$(grep -E '^User=' "$SERVICE_SRC" 2>/dev/null | head -n1 | cut -d= -f2 || true)"
...
echo "docker-install.sh: complete"
