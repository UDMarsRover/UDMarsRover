...
void loop() {
  ...
  //delay(1000);
  if (WAVE) {
    move_shoulder(MOTOR1_TIMER, MOTOR1_DIR, FREQ, UP, DOWN, LEFT, RIGHT, WAVE);
  ...
  if (Serial.available()) {
      ...
      switch (key) {
          case '1': UP = true; DOWN = false; LEFT = false; RIGHT = false; WAVE = false; STANDBY = false; break;
          case '2': DOWN = true; UP = false; LEFT = false; RIGHT = false; WAVE = false; STANDBY = false; break;
          case '3': LEFT = true; UP = false; DOWN = false; RIGHT = false; WAVE = false; STANDBY = false; break;
          case '4': RIGHT = true; UP = false; DOWN = false; LEFT = false; WAVE = false; STANDBY = false; break;
          case '5': STANDBY = true; UP = false; DOWN = false; LEFT = false; RIGHT = false; WAVE = false; break;
          case '6': WAVE = true; UP = false; DOWN = false; LEFT = false; RIGHT = false; STANDBY = false; break;
      }

      if ((UP) || (DOWN) || (LEFT) || (RIGHT) || (WAVE)) {
        FREQ = move_shoulder(MOTOR1_TIMER, MOTOR1_DIR, FREQ, UP, DOWN, LEFT, RIGHT, WAVE);
        update_duty(MOTOR1_CHANNEL, 50);
      }
      else if (STANDBY) {
...
uint32_t move_shoulder(ledc_timer_t TIMER, uint32_t DIRPIN, uint32_t FREQ, bool UP, bool DOWN, bool LEFT, bool RIGHT, bool WAVE)
{
  // update_duty(MOTOR1_CHANNEL, 50);
