...
echo "---Starting UDMRT Camera Pi Installation Script Part 2---"
echo "This script will take a long time."
echo "Network connection is required!"

# Make sure we are in the correct folder where the Dockerfile lives
cd /home/udmrt/opentera-docker

echo "Creating webrtc_subscriber.py node file..."

# Writes your python code directly to a file using a quoted here-doc
cat << 'EOF' > webrtc_subscriber.py
import rclpy
...
# Ensure user permissions are clean
chmod +x webrtc_subscriber.py

echo "Building the docker image..."
docker build -t opentera-ros .

echo "Starting container..."
# Because we mount $(pwd):/workspace, 'webrtc_subscriber.py' will be visible inside your container at /workspace/webrtc_subscriber.py
docker run -it --network host -v $(pwd):/workspace -w /workspace opentera-ros bash
